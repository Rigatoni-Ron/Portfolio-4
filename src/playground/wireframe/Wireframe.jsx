...
const CAM_D = 7.5
const CAM_F = 5.1
// Fixed reference range for the depth fade, so opacity doesn't re-normalise
// (and visibly pump) as the shape morphs between different depths.
const Z_RANGE = 1.35
...
export default function Wireframe({
  ...
  rings = 9,
  points = 72,
  segments = 8, // depth-fade slices per ring — more slices, smoother falloff
  verts = 8, // meridian lines connecting the rings
  // motion
  spin = 0.34, // turns per second
  tilt = 0.3, // radians on the X axis — positive looks down on the shape
  morphMs = 1500,
  ...
  useEffect(() => {
    ...
    const frame = (now) => {
      ...
      // Reuse one flat projected array: [screenX, screenY, depthZ] per point.
      ...
      for (let i = 0; i < R; i++) {
        ...
        for (let sIdx = 0; sIdx < o.segments; sIdx++) {
          ...
          el.setAttribute('d', d)
          el.style.opacity = Math.max(0.04, 1 - (zAvg + Z_RANGE) * fadeK)
        }
      }
      ...
      for (let c = 0; c < o.verts; c++) {
        ...
        const zAvg = zSum / R
        el.setAttribute('d', d)
        el.style.opacity = Math.max(0.04, 1 - (zAvg + Z_RANGE) * fadeK) * 0.7
      }
